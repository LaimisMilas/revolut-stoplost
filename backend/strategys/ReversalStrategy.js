const {shouldBuy, shouldSell} = require("./StrategyConditions");
const {sellState, getPosition, pushOrder} = require("../SellState");

/**
 * Strategijos pavadinimas: ReversalStrategy
 * Tikslas: Aprašyti pirkimo ir pardavimo logiką pagal pasirinktus indikatorius.
 */
const ReversalStrategy = async ({
                                                   candle,
                                                   analysis,
                                                   prevCandle,
                                                   prevAnalysis,
    ...
    // --- Pirkimo logika ---
    if (getPosition() === 0 && strategyCondition.buy(prevAnalysis, analysis, candle, prevCandle)) {
